<script lang="ts">
	...
	function toggleOpen(
		...
		if (!li) return;
		const height = getComputedStyle(li).getPropertyValue('--height');
...
<div class="bg-white p-16 rounded-lg flex gap-12 max-w-screen-lg mt-32">
	<h2 class="flex-grow flex-basis-1/4 text-3xl font-semibold">Questions and answers about login</h2>
	<ul class="flex-grow flex-basis-3/4 space-y-2">
		{#each accordion as open, i}
			...
			>
				...
				>
					Lorem ipsum dolor sit amet consectetur adipisicing elit. Labore dolores aperiam nisi illo
					recusandae, distinctio iure. Dolorem optio eum vitae!
				</p>
			</li>
		{/each}
	</ul>
